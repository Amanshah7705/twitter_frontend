...
export default function ShowTweet() {
  ...
  async function fetchTweetDetails() {
    try {
      const response = await axios.post(`${api}/tweet/getallfromthatuser`, {}, {
        headers: {
  ...
  async function removeLikes(id) {
    try {
      await axios.post(`${api}/likes/remove`, { whichtweet: id }, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
        },
  ...
  async function addLikes(id) {
    try {
      await axios.post(`${api}/likes/add`, { whichtweet: id }, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
        },
    ...
    <div className="max-w-2xl mx-auto mt-8 p-6 bg-white rounded-md shadow-md">
      ...
      {myTweets.length ? (
        <ul className="space-y-4">
          {myTweets.map((tweet) => (
            <li key={tweet._id} className="border p-4 rounded-md hover:shadow-md transition duration-300">
              <div className="text-xl font-semibold mb-2">{tweet?.tweetabout}</div>
              <div className="flex items-center space-x-2">
                <span className="text-gray-600">{tweet?.totallikes} Likes</span>
                {tweet?.LikedByme ? (
                  <button onClick={() => removeLikes(tweet._id)} className="text-red-500 hover:underline">Remove Like</button>
                ) : (
                  <button onClick={() => addLikes(tweet._id)} className="text-blue-500 hover:underline">Add Like</button>
                )}
              </div>
              <div className="mt-2">
                <button 
